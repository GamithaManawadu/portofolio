...
const Footer = () => {
  ...
  const textColor   = isDark ? "text-[#aaa6c3]" : "text-[#8a7a6a]";
  const borderColor = isDark ? "border-[#2a2a4a]" : "border-[#d8cfc0]";
  const iconHover   = isDark ? "hover:text-white" : "hover:text-[#1a1008]";

  return (
    <footer className={`w-full border-t ${borderColor} transition-colors duration-300`}>
      <div className={`max-w-7xl mx-auto px-6 sm:px-16 py-6 flex flex-col sm:flex-row justify-between items-center gap-4`}>

        <p
          className={`text-[13px] ${textColor}`}
          style={{ fontFamily: "'Poppins', sans-serif" }}
        >
          © {new Date().getFullYear()} Gamitha Manawadu
        </p>
        ...
        <div className="flex items-center gap-5">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className={`${textColor} ${iconHover} transition-colors duration-200`}
            aria-label="GitHub"
          >
            <GitHubIcon />
          </a>
